...
WebDriver driver;
	...
	/*
	 * public void ClickonGetStartedButton() throws InterruptedException { for(int
	 * i=0;i<datastructures.size();i++) { String
	 * dsname=datastructures.get(i).getText(); if(dsname.contains("Stack")) {
	...
	/*
	 * public void clearText() { Actions actions=new Actions(driver);
	 * actions.click(pythonCode).keyDown(Keys.CONTROL).sendKeys("a").keyUp(Keys.
	 * CONTROL).sendKeys(Keys.BACK_SPACE) .build().perform(); }
	 */
}
